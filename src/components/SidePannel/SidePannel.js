...
export function SidePannel() {
  const { labelList, setLabelList, selectedLabel, setSelectedLabel } =
    useLabel();
  ...
  function LableList() {
    return labelList.map((label) =>
      label === 'None' ? null : (
        <div
          className={`label ${label === selectedLabel ? 'label-active' : null}`}
  ...
  return (
    <div className="side-bar">
      {editLabelModal ? (
        <div className="edit-label-component-container" onClick={addNewLabel}>
          <EditLabel
            label={label}
            setLabel={setLabel}
